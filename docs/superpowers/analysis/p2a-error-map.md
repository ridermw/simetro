...
| Secret pattern in `raw_response` | Mitigated: pre-write redactor scans for known patterns (GitHub PAT, OAuth token, AWS key prefixes). Matches replaced with `<redacted: <pattern-name>>`. | Inspector: no signal; redaction is silent. AgentLog persists redacted text. | Test: write a `raw_response` containing each known pattern; assert the persisted text contains the redaction marker, not the original secret. |
...
| `crates/engine/tests/agent_log_v2.rs` (new in P2.A0.5) | §4: disk-full, permission-denied, path-traversal, mode 0600, truncation, redaction, concurrent-writer tests. |
...
- [x] §2 covers subprocess lifecycle hardening (matches PR #4 sec Finding 3).
- [x] §3 covers all four distinct JSON-parse sites with separate rescue semantics.
- [x] §4 covers AgentLog v2 write-path failures (matches PR #4 sec Finding 4).
