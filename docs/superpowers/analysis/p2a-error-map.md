...
| `SpawnFailed { kind: io::ErrorKind, message: String }` | `std::process::Command::spawn()` returns an `io::Error` for OS-level reasons (`ENOMEM`, FD exhaustion, sandbox-denies-exec, etc.). The binary IS present and resolvable; the OS refused to launch it. | `ExecutableMissing` (binary not found at all) AND `NotAuthenticated` (which would mislead the user toward `gh auth login` for a memory-exhaustion problem). |
...
| Subprocess spawn fails (out-of-FDs, ENOMEM, sandbox blocks exec) | `std::process::Command::spawn()` returns `io::Error`. | `SpawnFailed { kind, message }` → `Fault::AgentCrashed` (new variant per §1.1 — distinct from `NotAuthenticated` so the user is NOT misled into running `gh auth login` for an OS-level failure). | Bridge logs the io::Error with full context (`ErrorKind` enum value + message). Permanent failure for the run; no spawn-retry (auto-retry on OS resource exhaustion would just keep failing). | Inspector: red "Could not start Copilot subprocess (OS error: <kind> — <message>). Check available memory / file descriptor limits." | Test that wraps `spawn()` in a mock that returns each common `io::ErrorKind` (`OutOfMemory`, `PermissionDenied`, `Other`) and asserts the typed `SpawnFailed` is surfaced with the correct kind preserved (not flattened to `NotAuthenticated`). |
